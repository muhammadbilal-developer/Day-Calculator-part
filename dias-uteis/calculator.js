/**
 * Contador de Dias — Calculadora Dias Úteis / dias de trabalho (WordPress embed)
 */
(function () {
  "use strict";
  ...
  let state = { startDate: "", endDate: "", workingCountry: "br", includeCarnaval: false, result: null, loading: false, errors: {} };
  let pickers = [];
  ...
  function renderForm() {
    return `
      ...
      <button type="button" class="cdc-btn-primary" id="cdc-submit" ${state.loading ? "disabled" : ""}>
        ${state.loading ? `<i class="fa-solid fa-spinner cdc-spin"></i> ${STR.loading}` : STR.submit}
      </button>
  ...
  function bindFormEvents() {
    ...
    root.querySelector("#cdc-clear")?.addEventListener("click", () => {
      state = { startDate: "", endDate: "", workingCountry: "br", includeCarnaval: false, result: null, loading: false, errors: {} };
      render();
    });
